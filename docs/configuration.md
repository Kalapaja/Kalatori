...
| `shop_url` | Yes | — | Home URL of the shop. Shown on the payment page and returned in shop metadata to the front-end. |
| `reown_project_id` | Yes | — | Reown (formerly WalletConnect) project ID for wallet connection on the payment page. |
| `invoices_webhook_url` | No | `null` | URL where Kalatori sends invoice status updates (paid, expired, etc.). When unset, webhooks are not dispatched. |
...
| `ankr_api_token` | No | `null` | Ankr API token exposed to the payment front-end for RPC reads (supplementary to Reown). |
| `private_api_base_url` | No | falls back to `payments.payment_url_base` | URL the e-commerce backend plugin uses to reach the daemon's private API. Useful when the daemon is reachable from the shop on a different address than the public `payment_url_base`. |
| `shop_platform` | No | `"Unknown"` | Which e-commerce platform the daemon is paired with. Lets the admin API serve the matching integration plugin. See values below. |

**Accepted `shop_platform` values:**

- `"WooCommerce"` / `"woocommerce"` — the daemon knows it is paired with a WooCommerce shop and can serve the official plugin via the admin API.
- Any other string — recorded as an unsupported platform; no plugin is served.
- Omitted — treated as `Unknown`.

Currently WooCommerce is the only recognized value.
...
- **ankr_api_token**: Register at [ankr.com](https://www.ankr.com/) and create a project API key. Optional — wallet connect works without it, but front-end RPC reads are smoother with a dedicated token.
...
| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `chains.[ChainType].endpoints` | No | Built-in public RPCs | WebSocket RPC endpoints. Multiple entries enable automatic failover. Entries can be either plain URL strings or `{ "url": "...", "operations": [...] }` objects — see [Advanced endpoints](#advanced-endpoints). |
| `chains.[ChainType].assets` | No | Default asset per chain | Asset IDs to monitor. Cannot be overridden via env vars — only via JSON. |
...
Each entry in `endpoints` can be either a simple URL or an object that restricts which kinds of calls are sent to it. This is useful when one provider handles subscriptions (new-block streams) well while another is preferable for one-off requests (historical lookups, balance queries).
...
The `operations` array accepts `"Subscriptions"` and/or `"Requests"`. A plain string endpoint is treated as handling both. When multiple endpoints are eligible for the same operation, one is picked at random per call.

### etherscan_client.json

Configures the Polygonscan (Etherscan-family) REST client used to track token transfers on Polygon.
...
| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `api_key` | Yes | — | API key for Polygonscan, used to track token transfers. Always required — the daemon loads this config unconditionally at startup. If you only use Polkadot Asset Hub, set a placeholder value; the client is then created but its requests are never issued. |
...
Optional OAuth configuration for the daemon's admin API.

The whole config is optional, but if *any* part of it is present (either the file exists **or** any `KALATORI_AUTH_*` env var is set) then every required field below must be resolvable from JSON and/or env vars — missing or malformed required fields cause a startup panic. The config may be freely split between `auth.json` and env vars.

```json
{
  "auth_server_url": "https://auth.example.com",
  "client_id": "74484ae6-729f-4df1-ace4-7a195da232db",
  "client_secret": "BHscuFfKzJUoA9LuvaNoJSopPLLz3k86G1c-c73qjac",
  "token_public_keys": ["k4.public.LUu5hqL25TpEVkCnGgXt4hUyNK-MWcM0YW3geCHxaF0"],
  "base_url": "https://my-daemon.example.com"
}
```

**What this config does.** The daemon's admin API is gated by an OAuth 2.0 Authorization Code + PKCE flow. A client that wants to call the admin API initiates the flow at the daemon, which redirects the user to `auth_server_url` to sign in. On success the authorization server issues a PASETO v4.public access token (Ed25519-signed) back to the daemon, which the daemon verifies locally against `token_public_keys`. Token refresh is done server-to-server using `client_id` + `client_secret`. How the authorization server authenticates the end user (federated login, email link, etc.) is entirely on its side — the daemon trusts only the token signature and the `iss` / `aud` claims.

**Behavior when auth is disabled.** If neither `auth.json` nor any `KALATORI_AUTH_*` env var is present, auth is disabled and all `/admin` paths return 404.

| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `auth_server_url` | Yes | — | Base URL of the OAuth authorization server that signs tokens. Must match the `iss` claim in issued tokens. Normalized on load (lowercased host, trailing slash stripped). |
| `client_id` | Yes | — | OAuth client identifier assigned by the authorization server when the daemon is registered with it. |
| `client_secret` | Yes | — | Per-daemon shared secret for server-to-server calls (token exchange, refresh). Removed from environment after load and held in memory with zero-on-drop. |
| `previous_client_secret` | No | `null` | Previous value of `client_secret`. Accepted during a secret-rotation window so refreshes can continue briefly after a rotation. |
| `token_public_keys` | Yes | — | One or two PASERK v4 public keys (`k4.public.<base64url>`). Used to verify token signatures. Supplying two enables zero-downtime key rotation. |
| `clock_tolerance` | No | `30` (seconds) | Skew tolerance for `exp` and `iat` claim validation. |
| `base_url` | Yes | — | The daemon's own public base URL. Used to build the OAuth callback redirect URI that the authorization server sends the user back to. |

!!! warning "Security"
    `client_secret` and `previous_client_secret` are automatically removed from the environment after loading, and held in memory as `SecretString` (zeroized on drop).

### swaps.json

Optional configuration for external swap providers. Fully optional — all fields have defaults and may be omitted.

```json
{
  "bungee": {
    "api_key": "YOUR_BUNGEE_API_KEY",
    "affiliate": "YOUR_BUNGEE_AFFILIATE"
  },
  "zero_ex": {
...
| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `bungee` | No | `null` | Credentials for the Bungee swap aggregator. When unset, Bungee-powered swaps are unavailable. |
| `bungee.api_key` | Yes (if `bungee` is set) | — | Bungee API key. Held as `SecretString`. |
| `bungee.affiliate` | Yes (if `bungee` is set) | — | Bungee affiliate identifier. Held as `SecretString`. |
| `zero_ex` | No | Built-in default | Credentials and RPC for the 0x (ZeroEx) swap provider. |
| `zero_ex.api_key` | No | `""` (empty) | 0x API key. Held as `SecretString`. Empty means swap requests will be unauthenticated and may be rate-limited or rejected. |
| `zero_ex.rpc_url` | No | `https://polygon-bor-rpc.publicnode.com` | RPC endpoint used by the 0x client for on-chain reads. |
| `fees` | No | `null` | Reserved for future integrator-fee configuration (`fee_taker_address`, `fee_bps`). Currently not wired up — setting it has no effect. |
...
export KALATORI_SHOP_SHOP_URL="https://mystore.com"
...
# Auth (see auth.json)
export KALATORI_AUTH_AUTH_SERVER_URL="https://auth.example.com"
export KALATORI_AUTH_CLIENT_ID="your-client-id"
export KALATORI_AUTH_CLIENT_SECRET="your-client-secret"
export KALATORI_AUTH_TOKEN_PUBLIC_KEYS="k4.public.abc...,k4.public.def..."  # comma-separated list
export KALATORI_AUTH_BASE_URL="https://my-daemon.example.com"
```

!!! note
    Fields with nested structure (`recipient`, `slippage_params`, `chains`, `bungee`, `zero_ex`) should be configured via JSON files rather than environment variables. Flat fields of these structs are not reliably addressable via environment variables.
