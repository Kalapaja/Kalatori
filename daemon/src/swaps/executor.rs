...
use crate::dao::{DaoInterface, DAO, DaoSwapError};
use crate::clients::{AcrossClient, AcrossClientError, BungeeClient, BungeeClientError};
use crate::types::{CreateSwapData, InternalSwapDetails, SubmittedSwapParams, Swap, SwapExecutorType, SwapQuote, SwapSignatureParams};
...
impl<D: DaoInterface + 'static> SwapsExecutor<D> {
    ...
    pub async fn create_swap(&self, data: CreateSwapData) -> Result<Swap, SwapsExecutorError> {
        let quote_request_data = data.clone();

        let quote: SwapQuote = match data.swap_executor {
            SwapExecutorType::Across => self.across_client
                .get_swap_approval(quote_request_data.into())
                .await?
                .into(),
            SwapExecutorType::Bungee => self.bungee_client
        ...
        let created = self.dao
            .create_swap(swap)
            .await
            .map_err(|e| {
                match e {
                    DaoSwapError::InvoiceNotFound { invoice_id } => SwapsExecutorError::InvoiceNotFound { invoice_id },
    ...
    pub async fn submit_with_signature(
        ...
        let swap = self.dao
            .update_swap_set_signature(swap_signature.swap_id, swap_signature.signature)
            .await
            .map_err(|e| {
                match e {
                    DaoSwapError::NotFound { swap_id } => SwapsExecutorError::SwapNotFound { swap_id },
                    _ => SwapsExecutorError::DatabaseError,
                }
            })?;
        ...
        let response = self.bungee_client
    ...
    /// Mark swap as `Submitted` in database. Use this method for swaps which has been executed inside this service
    /// by either sending some API requests to executor or sent to blockhain directly. For swaps which has been
    /// sent on front-end use `update_swap_submitted_on_front_end` method.
    async fn update_swap_submitted_internally(&self, swap_id: Uuid) -> Result<Swap, SwapsExecutorError> {
        self.dao
    ...
    /// Mark swap as `Submitted` in database and update it's related transaction hash. Use this method for swaps
    /// which has been executed on front-end. For swaps which has been executed inside this service use
    /// `update_swap_submitted_internally` method.
    pub async fn update_swap_submitted_on_front_end(
        &self,
        submitted_swap: SubmittedSwapParams,
    ) -> Result<Swap, SwapsExecutorError> {
        // TODO: either use separate dao methods for different executors or move executor to the dao method too
        let SubmittedSwapParams { swap_id, swap_executor, transaction_hash } = submitted_swap;
